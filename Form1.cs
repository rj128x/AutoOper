...
namespace Modbus_TCP_Server
{
	public partial class Form1 : Form
	{
		...
		private int NUMBPR = -1;
		private DateTime PBRReadDate;
		...
		private void DBReadData() {
			...
			this.NUMBPR = ReadPBRNumberFromDB();

			this.DrawGTPS();
			Logger.Info("=finish DBReadData");
		}

		private int ReadPBRNumberFromDB() {
			int PBRNumber = -1;
			try {
				SqlCommand command = new SqlCommand(String.Format("SELECT TOP 1 * FROM PBRNUM WHERE DATA_DATE>'{0}' ORDER BY DATA_DATE DESC", DateTime.Now.Date.AddDays(-3).ToString("yyyy-MM-dd")), this.DBConn);
				SqlDataReader reader = command.ExecuteReader();
				while (reader.Read()) {
					PBRNumber = reader.GetInt32(1);
					PBRReadDate = reader.GetDateTime(0);
				}
				reader.Close();
				command.Dispose();
			} catch (Exception e) {
				Logger.Info("Ошибка при чтении номера ПБР в БД " + e.ToString());
			}
			toolStripStatusLabel3.Text = "  Номер ПБР: " + PBRNumber.ToString() + (isValidNumber() ? " (ПБР OK)" : " (ПБР ОШИБКА)");
			return PBRNumber;
		}

		private void WritePBRNumberToDB(int PbrNumber) {
			try {
				SqlCommand command = new SqlCommand(String.Format("INSERT INTO PBRNUM (data_date, num) VALUES ('{0}', {1})", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), PbrNumber), this.DBConn);
				command.ExecuteNonQuery();
				command.Dispose();
			} catch (Exception e) {
				Logger.Info("Ошибка при записи номера ПБР в БД " + e.ToString());
			}
			toolStripStatusLabel3.Text = "  Номер ПБР: " + NUMBPR.ToString() + (isValidNumber() ? " (ПБР OK)" : " (ПБР ОШИБКА)");
		}
		...
		private void Form1_Load(object sender, EventArgs e) {
			...
			this.dataGridView1.Rows[76].Cells[3].Value = "OUT--->Номер ПБР";
			this.dataGridView1.Rows[78].Cells[3].Value = "OUT--->Актуальный ПБР";
		...
		private void ParsePBR(string PBRFile) {
			...
			try {
				FileInfo fi = new FileInfo(PBRFile);
				string[] parts = fi.Name.Split(new char[] { '-' });
				for (int i = 0; i < parts.Count(); i++) {
					if (parts[i].Contains("pbr")) {
						string numStr = parts[i + 1];
						int num = Int32.Parse(numStr);
						if (num > 24)
							num = DateTime.Now.Hour;
						this.NUMBPR = num;
						this.PBRReadDate = DateTime.Now;
						WritePBRNumberToDB(num);
						break;
					}
				}
			} catch (Exception e) {
				Logger.Info("Ошибка при определении номера ПБР");
			}
		...
		private void timer1_Tick(object sender, EventArgs e) {
			...
			try {
				...
				if ((this.CurrSec > 10) && this.MySlave.connected) {
					bool ValOK = true;
					if (isGTP1)
						ValOK = ValOK && this.GTP1_final[this.CurrSec] >= 0;
					if (isGTP2)
						ValOK = ValOK && this.GTP2_final[this.CurrSec] >= 0;
					if (isGTP3)
						ValOK = ValOK && this.GTP3_final[this.CurrSec] >= 0;
					if (isGTP4)
						ValOK = ValOK && this.GTP4_final[this.CurrSec] >= 0;
					if (isGTP5)
						ValOK = ValOK && this.GTP5_final[this.CurrSec] >= 0;
					if (isGTP6)
						ValOK = ValOK && this.GTP6_final[this.CurrSec] >= 0;
					if (isGTP7)
						ValOK = ValOK && this.GTP7_final[this.CurrSec] >= 0;
					if (isGTP8)
						ValOK = ValOK && this.GTP8_final[this.CurrSec] >= 0;
					if (isGTP9)
						ValOK = ValOK && this.GTP9_final[this.CurrSec] >= 0;
					if (isGTP10)
						ValOK = ValOK && this.GTP10_final[this.CurrSec] >= 0;
					...
					this.MBRegisterWriteValue(77, (double)this.NUMBPR);
					this.MBRegisterWriteValue(79, this.isValidNumber() ? (double)1 : (double)0);
		...
		private bool isValidNumber() {
			try {
				return (this.PBRReadDate.AddHours(1).AddMinutes(10) > DateTime.Now);
			} catch {
				return false;
			}
		}
